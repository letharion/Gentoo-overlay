...
inherit eutils autotools flag-o-matic versionator depend.apache apache-module db-use libtool systemd git-r3
...
function php_get_uri ()
{
	case "${1}" in
		...
		;;
		"php-git")
			echo "git@github.com:php/php-src.git"
		;;
		*)
...
[[ ${PV} == ${PV/9999/} ]] || PHP_RELEASE="php-git"
PHP_P="${PN}-${PHP_PV}"
	...
	for sapi in ${SAPIS}; do
		if use "${sapi}" ; then
			...
			if [[ "${sapi}" == "apache2" ]] ; then
				...
				newins ".libs/libphp7$(get_libname)" "libphp${PHP_MV}$(get_libname)"
